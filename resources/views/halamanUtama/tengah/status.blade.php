<div class=" {{ Request::is('profile') ? 'mt-5 border-y border-y-slate-200' : 'mt-4 sm:mt-12 lg:mt-16 border-y border-y-slate-200' }}  py-5 px-0 ">
    ...
    <div class="flex  items-center ">
        ...
        <div class="ms-2 sm:ms-3 lg:ms-4 max-w-[85%] lg:max-w-[85%] me-2 lg:me-4">
            <div class="font-firaSans text-slate-900 text-xs sm:text-xl lg:text-base font-medium break-words sm:mb-1 lg:mb-0">
                <a href="
                @if ($status->user->id == auth()->user()->id)
                    /profile
                @else
            ...
            <span class="text-[0.53rem] sm:text-base lg:text-xs  text-slate-500 leading-none">
        ...
        @if (auth()->user()->id == $status->user_id)
            <div class="ms-auto  self-start place-items-center relative px-0 sm:px-2 lg:px-0">
    ...
    <div class="mt-6 sm:mt-11 lg:mt-10">
        <span class="font-poppins text-[0.73rem] sm:text-lg lg:text-base  text-slate-900 break-words">{{ $status->content }}</span>
    </div>
    {{-- akhir isi status --}}
